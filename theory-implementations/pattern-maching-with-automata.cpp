...
int TrancitionFunction[MAXN+1][NO_OF_CHARS]; 
...
void computeTF(string pat, int M, int TF[][NO_OF_CHARS])
{
    ...
    for (state = 0; state <= M; ++state)
        for (x = 0; x < NO_OF_CHARS; ++x){
            ...
            TrancitionFunction[state][x] = TF[state][x];
        }

}

// print for patterns A-Z
void printTF(int M){
    for (int state = 0; state <= M; ++state){
        for (char x = 'A'; x <= 'Z'; ++x){
            cout << TrancitionFunction[state][x] << " ";
        }
        cout << endl;
    }
}
...
// https://www.youtube.com/watch?v=kuMuFu9IRtw&list=PLTnlMZx2NlIEuxnMAU9vSPr4tuCTE06i9
/* Forning Quiz Teams. This DP solution will be explanied in Section 3.4 */
#include <iostream>
#include <algorithm>
#include <string.h>
#include <math.h>
...
using namespace std;
int N;
double dist[20][20] , memo[1 << 16]; // 1 << 16 is 2^16, recall max N = 8;
...
int main() {
	...
	while (sscanf(gets(line), "%d", &N), N) {
		...
		for (int i = 0; i < 2 * N; i++) //build pairwise distance table
			for (j = 0; j < 2 * N; j++)
